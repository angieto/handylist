...
class JobManager(models.Manager):
    def job_validator(self, postData):
        errors = {}
        if len(postData['title']) < 1 or len(postData['desc']) < 1 or len(postData['location']) < 1:
            errors['missing_fields'] = "All fields are required!"
        if len(postData['title']) < 4:
            errors['title_length'] = "Title must be longer than 3 characters"
        if len(postData['desc']) < 11:
            errors['desc_length'] = "Description must be longer than 10 characters"
        return errors
...
class Job(models.Model):
    ...
    creator = models.ForeignKey(User, on_delete=models.CASCADE, related_name="created_jobs")
    participants = models.ManyToManyField(User, related_name="participated_jobs")
    objects = JobManager()
